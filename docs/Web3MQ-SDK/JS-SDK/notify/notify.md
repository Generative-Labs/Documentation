...
const Child: React.FC<IProps> = (props) => {
  ...
  const handleEvent = (event: { type: string }) => {
    ...
    if (type === 'notification.getMyTopicList') {
      console.log('MyTopicList------', client.notify.myTopicList);
      setMyTopicList(client.notify.myTopicList);
    }
    if (type === 'notification.getMySubscribeList') {
      console.log('MySubscribeList------', client.notify.mySubscribeList);
      setMySubscribeList(client.notify.mySubscribeList);
    }
  };
  ...
  const handleCreate = async () => {
    const data = await client.notify.createTopic(topicName);
    console.log(data);
